...
export class BusOrderReportComponent implements OnInit {
  ...
  Print(depart) {
    this.busOrderReportParams.startDate = this.converDate.convertAB(
      $("#start").val()
    );
    this.busOrderReportParams.endDate = this.converDate.convertAB(
      $("#end").val()
    );
  ...
  loadReport() {
    this.reportService.getBusOrderReports(this.busOrderReportParams).subscribe(
      (res: PaginatedResult<BusOrder[]>) => {
