...
export class MealOrderReportComponent implements OnInit {
  ...
  Print(depart) {
    this.mealOrderReortParams.startDate = this.converDate.convertAB(
      $("#start").val()
    );
    this.mealOrderReortParams.endDate = this.converDate.convertAB(
      $("#end").val()
    );
  ...
  loadReport() {
    this.orderDetail = [];
    this.mealtype = [];
    this.reportService.getMealOrderReports(this.mealOrderReortParams).subscribe(
      (res: PaginatedResult<MealOrder[]>) => {
