...
export class BusVerificationReportComponent implements OnInit {
  ...
  Print() {
    this.busVerificationReportParams.startDate = this.converDate.convertAB(
      $("#start").val()
    );
    this.busVerificationReportParams.endDate = this.converDate.convertAB(
      $("#end").val()
    );
    this.loadReport();
  }
  ...
  loadReport() {
    ...
    this.reportService
      .getBusVerificationReports(this.busVerificationReportParams)
      .subscribe(
