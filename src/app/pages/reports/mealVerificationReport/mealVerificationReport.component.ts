...
export class MealVerificationReportComponent implements OnInit {
  ...
  Print() {
    this.mealVerificationReportParams.startDate = this.converDate.convertAB(
      $("#start").val()
    );
    this.mealVerificationReportParams.endDate = this.converDate.convertAB(
      $("#end").val()
    );
    this.loadReport();
  }
  ...
  loadReport() {
    ...
    this.reportService
      .getMealVerificationReports(this.mealVerificationReportParams)
      .subscribe(
