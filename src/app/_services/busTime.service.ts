...
export class BusTimeService {
   ...
   getBusTimes(page?, itemsPerPage?, BusTimeParams?): Observable<PaginatedResult<BusTime[]>> {
      const paginatedResult: PaginatedResult<BusTime[]> = new PaginatedResult<BusTime[]>();
      ...
      return this.http.get<BusTime[]>(this.baseUrl + 'BusTime/paged', { observe: 'response', params })
         .pipe(
            map(response => {
