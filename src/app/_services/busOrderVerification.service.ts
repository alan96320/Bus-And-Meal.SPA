...
export class BusOrderVerificationService {
  ...
  getBusOrderVerification(id: any): Observable<BusOrderVerification> {
    return this.http.get<BusOrderVerification>(this.baseUrl + 'BusOrderVerification/' + id);
  }
  ...
  getBusOrderVerifications(page?, itemsPerPage?, BusOrderVerificationParams?): Observable<PaginatedResult<BusOrderVerification[]>> {
    ...
    if (BusOrderVerificationParams != null) {
      if (BusOrderVerificationParams.date != null) {
        params = params.append('OrderEntryDate', BusOrderVerificationParams.date);
      }
      if (BusOrderVerificationParams.department != null) {
        params = params.append('DepartmentId', BusOrderVerificationParams.department);
      }
      if (BusOrderVerificationParams.OrderBy != null) {
        params = params.append('OrderBy', BusOrderVerificationParams.OrderBy);
        params = params.append('isDescending', BusOrderVerificationParams.isDesc);
      }
    }
    // console.log(BusOrderVerificationParams.date);

    return this.http.get<BusOrderVerification[]>(this.baseUrl + 'BusOrderVerification/paged', { observe: 'response', params })
      .pipe(
        map(response => {
